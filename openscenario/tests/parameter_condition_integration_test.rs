...
fn test_parameter_condition_end_to_end() {
    ...
    scenario
        ...
        .unwrap();
    scenario
        .add_parameter("VehicleState", ParameterType::String, "moving")
        .unwrap();
    scenario
        .add_parameter("DebugMode", ParameterType::Boolean, "false")
        .unwrap();
    ...
    let act_trigger = Trigger::new(ConditionGroup::new(vec![Condition::parameter(
        "MaxSpeed",
        "50.0",
        Rule::GreaterThan,
    )]));
    scenario
        .set_act_start_trigger("main_story", "main_act", act_trigger)
        .unwrap();

    // Add maneuver group and event
    scenario
        .add_maneuver_group("main_story", "main_act", "mg1")
        .unwrap();
    scenario
        .add_actor("main_story", "main_act", "mg1", "ego")
        .unwrap();
    scenario
        .add_maneuver("main_story", "main_act", "mg1", "maneuver1")
        .unwrap();
    ...
    let event_trigger = Trigger::new(ConditionGroup::new(vec![Condition::parameter(
        "VehicleState",
        "moving",
        Rule::EqualTo,
    )]));
