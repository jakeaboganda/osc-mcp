...
fn test_multiple_actions_per_entity_export() {
    ...
    assert!(xml.contains("LaneChangeAction"), "LaneChangeAction not found in XML");
...
fn test_multiple_actions_same_event() {
    ...
    assert!(xml.contains("LaneChangeAction"), "LaneChangeAction not found");
    
    // Should have ONE event with BOTH actions
    let event_count = xml.matches("<Event name=\"combined_event\"").count();
    assert_eq!(event_count, 1, "Expected exactly 1 event, found {}", event_count);
