...
fn test_validation_workflow() {
    ...
    let validation_result = handle_validate_scenario(
        state.clone(),
        scenario_id.clone(),
    ).expect("Failed to validate scenario");
    
    // Parse validation result as JSON
    let report: serde_json::Value = serde_json::from_str(&validation_result)
        .expect("Validation result should be valid JSON");
    
    // Check that we got a valid report structure
    assert!(report.get("valid").is_some(), "Report should have 'valid' field");
    assert!(report.get("errors").is_some(), "Report should have 'errors' field");
    
    // The scenario should be valid (basic structure is correct)
    let is_valid = report["valid"].as_bool().unwrap_or(false);
    assert!(is_valid, "Basic scenario should be valid. Errors: {}", report["errors"]);
}
...
"#;
    ...
    assert!(xml_content.contains("CatalogReference"), "XML should contain CatalogReference");
    assert!(xml_content.contains("sports_car"), "XML should reference the vehicle entity");
}
